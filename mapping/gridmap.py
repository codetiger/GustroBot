...
class GridMap:
    ...
    def _calc_grid_map_config(self, ox, oy, xyreso):
        """
    ...
    def _atan_zero_to_twopi(self, y, x):
        angle = math.atan2(y, x)
        if angle < 0.0:
            angle += math.pi * 2.0
        return angle

    def _init_floodfill(self, cpoint,  opoints, xypoints, mincoord, xyreso):
        """
        cpoint: center point
        ...
        for (x, y) in zip(ox, oy):
            ...
            free_area = self._bresenham((prev_ix, prev_iy), (ix, iy))
            for fa in free_area:
                if fa[0] < xw and fa[1] < yw:
                    pmap[fa[0]][fa[1]] = 0  # free area 0.0
            prev_ix = ix
            prev_iy = iy
        return pmap

    def _flood_fill(self, cpoint, pmap):
        """
    ...
    def _generate_ray_casting_grid_map(self, ox, oy, xyreso, breshen=True):
        ...
        """
        minx, miny, maxx, maxy, xw, yw = self._calc_grid_map_config(ox, oy, xyreso)
        pmap = (np.ones((xw, yw))) * 0.5 # default 0.5 -- [[0.5 for i in range(yw)] for i in range(xw)] 
        ...
        if breshen:
            for (x, y) in zip(ox, oy):
                ix = int(round((x - minx) / xyreso)) # x coordinate of the the occupied area
                iy = int(round((y - miny) / xyreso)) # y coordinate of the the occupied area
                laser_beams = self._bresenham((centix, centiy), (ix, iy)) # line form the lidar to the cooupied point
                for laser_beam in laser_beams:
                    pmap[laser_beam[0]][laser_beam[1]] = 0.0 # free area 0.0
                if (ix+1) < xw and iy < yw:
                    pmap[ix+1][iy] = 1.0   # extend the occupied area
                if ix < xw and (iy+1) < yw:
                    pmap[ix][iy+1] = 1.0   # extend the occupied area
                if (ix+1) < xw and (iy+1) < yw:
                    pmap[ix+1][iy+1] = 1.0 # extend the occupied area
        # occupancy grid computed with with flood fill
        else:
            pmap = self._init_floodfill((centix, centiy), (ox, oy), (xw, yw),  (minx, miny), xyreso)
            self._flood_fill((centix, centiy), pmap)
    ...
    def addPoint(self, center, angle, distance):
        # if distance > 150:
        #     return
        self._pointx.append(center[0] + math.cos(angle) * distance)
        self._pointy.append(center[1] + math.sin(angle) * distance)
    
    def recalculate(self):
        ox = np.array(self._pointx)
        oy = np.array(self._pointy)
        self._pmap, minx, maxx, miny, maxy, xyreso = self._generate_ray_casting_grid_map(ox, oy, self._resolution, False)
        
    def saveImage(self, filename):
        ox = np.array(self._pointx)
        oy = np.array(self._pointy)
        xyres = np.array(self._pmap).shape
        
        plt.figure(1, figsize=(100,40))
        plt.subplot(122)
        plt.imshow(self._pmap, cmap="PiYG_r")
        ...
        figure = plt.gcf()
        figure.savefig(filename, dpi=72)
